...
export const generateTeam = async (req, res) => {
  try {
    ...
    if (!startingMembers || startingMembers.length === 0) {
      startingMembers = findAndSetAStartingMember(startingMembers, collectionsByRole, neededRoles);
    } else {
      collectionsByRole = filterOutStartingMembersFromCollections(startingMembers, collectionsByRole);
    }
    ...
    const { engineers, designers, product } = sortMembersByRole(finalTeam);
    ...
    if (daysInCommon < 3) {
      throw new Error(
        `Team has ${response.totalCommonHours} overlapping availability each week, but only over${daysInCommon} day(s).`,
      );
    }
