...
const minimumHoursOverlapRequired = 3;
...
export const getNeededMembersByRoleWithMostOverlap = async (
  ...
  const meetsMinOverlap =
    neededRoles[roleNickName] > 0
      ? meetMinimumOverlappingHours(startingMembers, collection).sort((a, b) => b.commonHours - a.commonHours)
      : [];
  ...
  if (meetsMinOverlap.length < neededRoles[roleNickName]) {
    ...
    throw new Error(
      `None of set (count: ${count}, offset: ${offset}) of ${role}s matches minumum availability. Try again with offset: ${nextOffset}`,
    );
  }
...
export const findAndSetAStartingMember = (startingMembers, collectionsByRole, neededRoles) => {
