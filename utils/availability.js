...
// }
export const generateRealisticSingleDayAvailability = () => {
  //to choose at least two time slot to be sure that user select at least one hour
  const numberOfSlots = 2 + randomIndex(commonAvailableTimeslots.length - 2);
  const copyAvailableTimeSlots = [...commonAvailableTimeslots];
  const timeSlots = [];
  for (let i = 0; i < numberOfSlots; i++) {
    const index = randomIndex(copyAvailableTimeSlots.length);
    timeSlots.push(copyAvailableTimeSlots[index]);
    //remove the chosen timeslot to ensure picking a unique timeslot
    copyAvailableTimeSlots.splice(index, 1);
  }
  return timeSlots;
};
/**
...
export const findCommonAvailability = (members) => {
  ...
  weekdays.forEach((day) => {
    ...
    if (sharedDayAvail.length >= 2) {
      logicalCommonAvailability[day] = sharedDayAvail;
    } else {
      logicalCommonAvailability[day] = [];
    }
  });
